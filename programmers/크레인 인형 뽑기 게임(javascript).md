...
`javascript`의 고유 특성을 살려 풀어보려고 했지만 아직 내공이 부족했습니다. `java` 혹은 `c++` 스타일과 다를 바 없어서 찝찝합니다. 이전에 `java`로 풀어봤을 때랑 다른 부분이 거의 없습니다. 다른 사람의 풀이를 보면 기발한 코드 혹은 `javascript`스러운 코드가 보입니다만, 제가 작성한 것이 속도도 빠르고 메모리도 덜 먹습니다. 일단 위안 삼아 봅니다.

#### 코드

```javascript
function solution(board, moves) {
  let newMoves = moves.map((m) => m - 1);
  let answer = 0;
  let stack = [];
  for (let i = 0; i < newMoves.length; ++i) {
    for (let j = 0; j < board.length; ++j) {
      if (board[j][newMoves[i]] !== 0) {
        const val = board[j][newMoves[i]];
